...
export default function TriagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const sendMessage = useCallback(async (text: string) => {
    if (!text.trim() || streaming || !currentStory) return
    ...
    const res = await fetch(`/api/projects/${projectId}/triage-chat`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        item: { title: currentStory.title, userStory: currentStory.userStory, notes: currentStory.notes },
        messages: next,
      }),
    })
  ...
  }, [streaming, currentStory, messages, projectId])
  ...
  async function applyDecision() {
    ...
    if (deciding === 'epic') {
      ...
      // Stay as story, just update parent assignments
    ...
    setSaving(false)
    resetForNext()
    // Stay at same idx; next item slides in
  }
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-4 shrink-0">
        ...
        <div className="ml-auto flex items-center gap-3">
          <div className="text-xs text-gray-500">
            {totalDone} done · {remaining.length} remaining
          </div>
      ...
      <div className="flex flex-1 min-h-0 gap-0">
        ...
        <div className="w-[420px] shrink-0 flex flex-col border-r border-gray-200 bg-white">
          ...
          <div className="p-6 border-b border-gray-100">
            ...
            {story.userStory && (
              <p className="text-sm text-gray-600 leading-relaxed line-clamp-4">{story.userStory}</p>
            )}
            {story.notes && !story.userStory && (
              <p className="text-sm text-gray-400 italic line-clamp-3">{story.notes}</p>
            )}
          </div>
          ...
          {deciding && (
            <div className="px-6 py-4 border-b border-gray-100 space-y-2">
              {(deciding === 'feature' || deciding === 'story') && (
                <div>
                  <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide block mb-1">
                    Parent Epic <span className="font-normal normal-case">(optional)</span>
                  </label>
                  <select
                    value={targetEpicId}
                    onChange={e => { setTargetEpicId(e.target.value); setTargetFeatureId('') }}
              ...
              {deciding === 'story' && (
                <div>
                  <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide block mb-1">
                    Parent Feature <span className="font-normal normal-case">(optional)</span>
                  </label>
                  <select
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
                ...
                <p className="text-gray-500 text-sm max-w-xs">
                  Ask Claude to help classify this item. Try: <em>"What makes this an epic vs a feature?"</em> or <em>"This touches payments and notifications, how should I split it?"</em>
                </p>
                <div className="grid grid-cols-1 gap-1.5 mt-2 w-full max-w-xs">
                  {[
                    'Is this too big to be a single story?',
                    'What would the sub-stories look like?',
                    'Which level best matches this scope?',
                  ].map(q => (
                    <button
                      key={q}
                      onClick={() => sendMessage(q)}
                      className="text-xs text-indigo-600 border border-indigo-200 rounded-lg px-3 py-1.5 hover:bg-indigo-50 text-left"
                    >
                      {q}
                    </button>
                  ))}
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                >
                  {msg.content || <span className="text-gray-400 italic">…</span>}
                </div>
              </div>
            ))}
            <div ref={chatEndRef} />
          ...
          <div className="border-t border-gray-200 bg-white px-4 py-3">
            <div className="flex items-end gap-2">
              <textarea
                ...
                rows={2}
                disabled={streaming}
                className="flex-1 resize-none border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 disabled:bg-gray-50"
              />
              <button
                onClick={() => sendMessage(input)}
                disabled={!input.trim() || streaming}
                className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed h-[42px]"
              >
                {streaming ? '…' : 'Send'}
              </button>
            </div>
